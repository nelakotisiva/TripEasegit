...
<style>
    ...
    .page-container { max-width: 950px; margin: 0 auto; padding: 10px; }
    .header { background:#0d6efd; color:white; padding:15px; font-size:26px; text-align:center; }
    .search-box { width:100%; text-align:center; margin:20px 0; }
    input { padding:10px; margin:5px; width:200px; border:2px solid #0d6efd; border-radius:8px; }
    button { padding:10px 20px; background:#0d6efd; color:white; border:none; border-radius:8px; cursor:pointer; }
    button:hover { background:#004ab9; }
    .table-wrapper { width: 100%; overflow-x: auto; margin-top: 20px; }
    table { width:100%; min-width: 800px; border-collapse:collapse; background: white; border-radius: 10px; overflow: hidden; }
    th { background:#0d6efd; color:white; padding:12px; position: sticky; top: 0; }
    td { padding:12px; text-align:center; border-bottom:1px solid #ccc; }
    tbody tr:hover { background: #f0f7ff; }
    .msg { text-align:center; margin-top:10px; color:green; font-weight:bold; }
...
<div class="page-container">
    ...
    <%
        ...
        boolean userSearched = (src != null && !src.trim().isEmpty() && dest != null && !dest.trim().isEmpty());
    %>
            ...
            <tr>
                ...
                <td>
                    <% if (f.getSeatsAvailable() > 0) { %>
                        <form action="BookFlight" method="post" class="inline">
                            <input type="hidden" name="flightId" value="<%= f.getFlightId() %>">
                            <input type="number" name="seats" value="1" min="1" max="<%= f.getSeatsAvailable() %>">
